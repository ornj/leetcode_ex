defmodule Leetcode.GroupPeopleByGroupSize do
  @moduledoc """
  There are n people whose IDs go from 0 to n - 1 and each person belongs exactly to one
  group. Given the array groupSizes of length n telling the group size each person
  ...
  def group_the_people(group_sizes) do
    ...
    |> Enum.reduce(%{}, fn {size, index}, acc ->
        Map.update(acc, size, [index], &(&1 ++ [index]))
      end)
    |> Map.to_list()
